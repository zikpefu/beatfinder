import sys, json, subprocess, requests

fp = subprocess.run(["./fpcalc",sys.argv[1]], capture_output=True, text=True)

base_url = "https://api.acoustid.org/v2/lookup"
#can/should be hidden or obscured
KEY_attach = "client=HJUyBTD1Pr"

#other meta data
#META_attach = "meta=recordings+releasegroups+compress"
META_attach = "meta=recordingids"
#taken from ./fpcalc

if fp.stderr == "":
    #no error in fpcalc
    fp_remove_nl = fp.stdout.replace('\n','&')
    fp_slice = fp_remove_nl[:-1]
    split_list = fp_slice.split('&')

    corrected_list = []
    url = base_url + '?' + KEY_attach + '&' + META_attach
    for entry in split_list:
        eq_find = entry.find('=')
        new_append = entry[0:eq_find].lower() + entry[eq_find:]
        url = url + '&' + new_append

    payload={}
    headers = {}
    response = requests.request("GET", url, headers=headers, data=payload)
    #print(response.text)
    json_object = json.loads(response.text)

    #json_formatted_str = json.dumps(json_object, indent=2)

    #print(json_formatted_str)

    #print first entry of API
    base_musicrainz_url = "http://musicbrainz.org/ws/2/recording"

    musicBranizID = list(json_object['results'][0]['recordings'][0].values())
    MUSICBRAINZID_attach = musicBranizID[0]
    INC_attach = "inc=artist-credits"
    JSON_attach = "fmt=json"
    musicbrainz_url = base_musicrainz_url + '/' + MUSICBRAINZID_attach + '?' + INC_attach + '&' + JSON_attach
    response = requests.request("GET", musicbrainz_url, headers=headers, data=payload)

    json_object = json.loads(response.text)

    json_formatted_str = json.dumps(json_object, indent=2)

    song_name = json_object['title']
    song_artist = json_object["artist-credit"][0]["name"]
    print('Found a song: ' + song_name + ' - ' + song_artist)








else:
    print("error in fpcalc\n")

#User/application for metabraniz
#beatfinder/1.0 ( http://beatfinder.tech/demo/ )

#http://musicbrainz.org/ws/2/recording/21b3d57a-90fc-4cb5-b651-24ed4e874f0e?inc=artist-credits&fmt=json

#url = "duration=641&fingerprint=AQABz0qUkZK4oOfhL-CPc4e5C_wW2H2QH9uDL4cvoT8UNQ-eHtsE8cceeFJx-LiiHT-aPzhxoc-Opj_eI5d2hOFyMJRzfDk-QSsu7fBxqZDMHcfxPfDIoPWxv9C1o3yg44d_3Df2GJaUQeeR-cb2HfaPNsdxHj2PJnpwPMN3aPcEMzd-_MeB_Ej4D_CLP8ghHjkJv_jh_UDuQ8xnILwunPg6hF2R8HgzvLhxHVYP_ziJX0eKPnIE1UePMByDJyg7wz_6yELsB8n4oDmDa0Gv40hf6D3CE3_wH6HFaxCPUD9-hNeF5MfWEP3SCGym4-SxnXiGs0mRjEXD6fgl4LmKWrSChzzC33ge9PB3otyJMk-IVC6R8MTNwD9qKQ_CC8kPv4THzEGZS8GPI3x0iGVUxC1hRSizC5VzoamYDi-uR7iKPhGSI82PkiWeB_eHijvsaIWfBCWH5AjjCfVxZ1TQ3CvCTclGnEMfHbnZFA8pjD6KXwd__Cn-Y8e_I9cq6CR-4S9KLXqQcsxxoWh3eMxiHI6TIzyPv0M43YHz4yte-Cv-4D16Hv9F9C9SPUdyGtZRHV-OHEeeGD--BKcjVLOK_NCDXMfx44dzHEiOZ0Z44Rf6DH5R3uiPj4d_PKolJNyRJzyu4_CTD2WOvzjKH9GPb4cUP1Av9EuQd8fGCFee4JlRHi18xQh96NLxkCgfWFKOH6WGeoe4I3za4c5hTscTPEZTES1x8kE-9MQPjT8a8gh5fPgQZtqCFj9MDvp6fDx6NCd07bjx7MLR9AhtnFnQ70GjOcV0opmm4zpY3SOa7HiwdTtyHa6NC4e-HN-OfC5-OP_gLe2QDxfUCz_0w9l65HiPAz9-IaGOUA7-4MZ5CWFOlIfe4yUa6AiZGxf6w0fFxsjTOdC6Itbh4mGD63iPH9-RFy909XAMj7mC5_BvlDyO6kGTZKJxHUd4NDwuZUffw_5RMsde5CWkJAgXnDReNEaP6DTOQ65yaD88HoeX8fge-DSeHo9Qa8cTHc80I-_RoHxx_UHeBxrJw62Q34Kd7MEfpCcu6BLeB1ePw6OO4sOF_sHhmB504WWDZiEu8sKPpkcfCT9xfej0o0lr4T5yNJeOvjmu40w-TDmqHXmYgfFhFy_M7tD1o0cO_B2ms2j-&client=HJUyBTD1Pr"
